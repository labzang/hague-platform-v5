...
from labzang.apps.data.geospatial.application.ports import (
    SeoulCrimePort,
...
# --- SeoulPreprocessorPort ---
class SeoulPreprocessorAdapter(SeoulPreprocessorPort):
...
# --- GeocodePort (카카오맵) ---
class KakaoGeocodeAdapter(GeocodePort):
    ...
    def _retrieve_api_key(self) -> str:
        ...
        try:
            ...
            from labzang.core.paths import LEARNING_ROOT, LABZANG_ROOT

            for p in [
                LEARNING_ROOT / ".env",
                LABZANG_ROOT / ".env",
                (LABZANG_ROOT.parent / ".env").resolve(),
            ]:
                if p.exists():
                    load_dotenv(p)
                    break
