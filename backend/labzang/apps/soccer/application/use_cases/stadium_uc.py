...
from labzang.apps.soccer.application.orchestrators.states.stadium_state import (
    StadiumProcessingState,
)
from labzang.apps.soccer.application.services.stadium_service import StadiumService
...
class StadiumUC:
    ...
    def __init__(
        ...
        logger.info("[오케스트레이터] StadiumUC 초기화 완료 (중앙 MCP 서버 사용)")
    ...
    async def process_query(self, question: str) -> Dict[str, Any]:
        ...
        Returns:
            처리 결과 딕셔너리
        """
        logger.info(f"[StadiumUC] 질문 수신: {question}")
        print(f"[StadiumUC] 사용자 질문: {question}")
        ...
        logger.info(f"[StadiumUC] 질문 처리 완료: {question}")
        return result
