...
class EmbeddingClient:
    ...
    한국어 축구 데이터에 특화된 임베딩 모델을 사용합니다.
    추천 모델: jhgan/ko-sroberta-multitask (768차원, 한국어 특화)
    """
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        if model_path is None:
            ...
            # 환경 변수가 없으면 기본 경로 사용
            if model_path is None:
                # artifacts/embedding-models/jhgan--ko-sroberta-multitask
                ...
                model_path = (
                    BACKEND_ROOT / "artifacts" / "embedding-models" / model_dir_name
                )
    ...
    def _initialize_embeddings(self):
        ...
        try:
            ...
            if self.model_path and self.model_path.exists():
                ...
                logger.info(
                    "[임베딩 클라이언트] 모델이 artifacts 폴더에 자동으로 다운로드됩니다."
                )

            # HuggingFaceEmbeddings 초기화
            # cache_folder를 artifacts/embedding-models로 설정하여 로컬에 저장
            cache_folder = BACKEND_ROOT / "artifacts" / "embedding-models"
