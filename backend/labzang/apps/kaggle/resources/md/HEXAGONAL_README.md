...
**진입점**: `backend/main.py`에서 hex 타이타닉 라우터를 `prefix="/hex"`로 등록합니다.
...
- 챗봇 API와 함께 헥사고날 타이타닉 API가 **`/hex/`** prefix로 등록됩니다. (`/hex/titanic/` 등)
...
- **새 API**: `adapter/input/api/v1/`에 라우터 추가 후 `backend/main.py`에서 등록.
