"""
타이타닉 아웃바운드 어댑터 (한 파일에 통합)
- ITitanicDataPort, IPreprocessorPort, IModelRunnerPort 구현
"""
...
# --- ITitanicDataPort 구현 ---
class CsvTitanicDataAdapter(ITitanicDataPort):
    ...
    def save_submission(self, passenger_ids: Any, predictions: Any) -> str:
        ...
        pd.DataFrame({"PassengerId": passenger_ids, "Survived": predictions.astype(int)}).to_csv(
            out, index=False
        )
        return str(out)


# --- IPreprocessorPort 구현 ---
class TitanicPreprocessorAdapter(IPreprocessorPort):
    def preprocess(self, train_df: Any, test_df: Any) -> TitanicDataSet:
        ...
        for df in (train, test):
            ...
            df["Fare"] = pd.qcut(df["Fare"].fillna(df["Fare"].median()), 4, labels=[1, 2, 3, 4]).astype(float)
            df["Fare"] = df["Fare"].fillna(1)
            df["Age"] = df["Age"].fillna(-0.5)
            bins, labels = [-1, 0, 5, 12, 18, 24, 35, 60, np.inf], [0, 1, 2, 3, 4, 5, 6, 7]
            df["Age"] = pd.cut(df["Age"], bins=bins, labels=labels).astype(float).fillna(0)
            if "Name" in df.columns:
                ...
                df["Title"] = df["Title"].map({"Mr": 1, "Ms": 2, "Mrs": 3, "Master": 4, "Royal": 5, "Rare": 6}).fillna(0).astype(int)
                df.drop(columns=["Name"], inplace=True)
            for c in list(df.columns):
                if df[c].dtype == "object" or getattr(df[c].dtype, "name", "") == "category":
                    df.drop(columns=[c], inplace=True, errors="ignore")
        return TitanicDataSet(train=train, test=test)


# --- IModelRunnerPort 구현 ---
class SklearnTitanicModelAdapter(IModelRunnerPort):
    ...
    def evaluate(self, train_data: Any, target_column: str) -> Dict[str, Any]:
        ...
        for name, clf in models.items():
            try:
                acc = round(float(np.mean(cross_val_score(clf, X, y, cv=kf, n_jobs=1, scoring="accuracy"))) * 100, 2)
    ...
    def predict_for_submit(self, train_data: Any, test_data: Any, target_column: str) -> Any:
        X_train = train_data.drop(columns=[target_column]).select_dtypes(include=[np.number])
