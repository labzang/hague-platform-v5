...
export interface UserState {
  user: UserInfo | null;
  isLoggedIn: boolean;
  accessToken: string | null; // 메모리에만 저장 (persist 제외)
}
...
> = (set, get) => ({
  ...
  logout: () => {
    ...
    set((state) => ({ 
      user: { ...state.user, user: null, isLoggedIn: false, accessToken: null } 
    }));
    console.log('[userSlice] Zustand 상태 초기화 완료');
    
    // localStorage에서 토큰 및 persist된 상태 삭제 (클라이언트 측만 정리)
    if (typeof window !== 'undefined') {
      // refresh_token과 auth_provider는 localStorage에 저장 가능 (access_token은 메모리에만)
